...
var _selected_car_scene: PackedScene = null


func _ready() -> void:
	add_to_group(&"car_select")
...
func _load_scene(car_scene: PackedScene) -> void:
	_selected_car_scene = car_scene

	# Show loading screen and wait for it to be rendered.
	loading_screen.visible = true
	await RenderingServer.frame_post_draw

	if button_race_mode.button_pressed:
		_load_race_scene(car_scene)
	else:
		_load_town_scene(car_scene)

	hide()


func _load_town_scene(car_scene: PackedScene) -> void:
	var car: Node3D = car_scene.instantiate()
	car.name = "car"
	_town = preload("res://town/town_scene.tscn").instantiate()

	if button_sunrise.button_pressed:
		_town.mood = _town.Mood.SUNRISE
	elif button_day.button_pressed:
		_town.mood = _town.Mood.DAY
	elif button_sunset.button_pressed:
		_town.mood = _town.Mood.SUNSET
	elif button_night.button_pressed:
		_town.mood = _town.Mood.NIGHT

	_town.setup(car, _on_back_pressed, button_sdfgi.button_pressed)

	get_parent().add_child(_town)


func _load_race_scene(car_scene: PackedScene) -> void:
	var car: Node3D = car_scene.instantiate()
	car.name = "car"
...
func _on_back_pressed() -> void:
	if is_instance_valid(_race_scene):
		# Currently in a race, go back to main menu.
		_race_scene.queue_free()
		_race_scene = null
		loading_screen.visible = false
		show()
		focus_first_car()
	elif is_instance_valid(_town):
...
## Called by the race manager when the player wants to restart.
func restart_race() -> void:
	if is_instance_valid(_race_scene):
		_race_scene.queue_free()
		_race_scene = null

	if _selected_car_scene:
		await RenderingServer.frame_post_draw
		_load_race_scene(_selected_car_scene)


## Called by the race manager when the player wants to return to the menu.
