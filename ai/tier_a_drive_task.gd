extends BTAction
## Tier A driving policy (see ai/COMPARISON.md): drives a VehicleBody3D along
## a Path3D curve using steering and throttle control. Reads "path" (Path3D)
## and "car" (VehicleBody3D) from the blackboard -- unlike the sensor-only
## policy in ai/ai_drive_task.gd (Tier B), this one has privileged access to
## the track's exact curve, which a human player doesn't have.
##
